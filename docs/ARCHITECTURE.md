...
Пошаговые сценарии работы токенов вынесены в [JWT_FLOW](./JWT_FLOW.md).  
Модель безопасности, угрозы и ограничения вынесены в [SECURITY_MODEL](./SECURITY_MODEL.md).  
Локальный запуск и команды разработки описаны в [DEVELOPMENT](./DEVELOPMENT.md).
